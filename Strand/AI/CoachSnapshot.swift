...
enum CoachSnapshotBuilder {
    static func build(repository: Repository, question: String? = nil,
        ...
        let readiness = ReadinessEngine.evaluate(days: sorted, today: latest?.day)
        ...
        let latestSleep = sleepSession.max { $0.endTs < $1.endTs }
        ...
        let sleep = CoachSnapshot.Sleep(
            ...
            bedtime: latestSleep.map { Date(timeIntervalSince1970: TimeInterval($0.effectiveStartTs)) },
            wakeTime: latestSleep.map { Date(timeIntervalSince1970: TimeInterval($0.endTs)) },
            naps: max(0, sleepSession.filter {
                now.timeIntervalSince1970 - Double($0.endTs) <= 7 * 86_400
            }.count - 7),
            debtTrend: trend(debtSeries.map(\.value)))

        let workouts = await repository.workoutRows(days: 28)
        let cutoff7 = Int(now.addingTimeInterval(-7 * 86_400).timeIntervalSince1970)
        let cutoff28 = Int(now.addingTimeInterval(-28 * 86_400).timeIntervalSince1970)
        let rows7 = workouts.filter { $0.startTs >= cutoff7 }
        let rows28 = workouts.filter { $0.startTs >= cutoff28 }
        let strains7 = rows7.compactMap(\.strain)
        let strains28 = rows28.compactMap(\.strain)
        let acute = mean(strains7)
        ...
        let training = CoachSnapshot.Training(
            ...
            acuteLoad: acute,
            chronicLoad: chronic,
            acuteChronicRatio: readiness.acwr
                ?? zipOptional(acute, chronic).flatMap { pair in
            ...
            completeness: Double(sorted.suffix(28).count) / 28)
        ...
        return CoachSnapshot(
            ...
            sleep: sleep, training: training, strength: strength)
    }
    ...
    private static func buildStrength(repository: Repository, question: String?,
        ...
        let startDay = Repository.localDayKey(start)
        let daily = (try? await store.dailyMuscleLoads(
            deviceId: repository.deviceId, from: startDay, to: startDay)) ?? []
        let residualByMuscle = Dictionary(
            residual.map { ($0.muscleId, $0.residualLoad) },
            uniquingKeysWith: max)
        let muscles = daily.sorted { $0.normalizedLoad > $1.normalizedLoad }.prefix(8).map {
            CoachSnapshot.Strength.Muscle(
                id: $0.muscleId, load: $0.normalizedLoad,
                residual: residualByMuscle[$0.muscleId], confidence: $0.confidence)
        }
    ...
    private static func metric(_ values: [(day: String, value: Double)]) -> CoachSnapshot.Metric {
        let numbers = values.map(\.value).filter(\.isFinite)
        let current = numbers.last
        let avg7 = mean(Array(numbers.suffix(7)))
        let avg30 = mean(Array(numbers.suffix(30)))
        ...
        return .init(
            ...
            trend: trend(Array(numbers.suffix(7))),
            availableCount: min(30, numbers.count), expectedCount: 30,
            source: "on-device merged wearable data", timeRange: "current / 7 days / 30 days")
    }
    ...
    private static func consecutiveHardDays(_ days: [DailyMetric]) -> Int {
        var count = 0
        for day in days.reversed() {
            guard (day.strain ?? 0) >= 70 else { break }
            count += 1
        }
        return count
    }
...
enum CoachSnapshotFormatter {
    static func format(_ snapshot: CoachSnapshot) -> String {
    ...
    private static func metric(_ title: String, _ value: CoachSnapshot.Metric,
                               unit: String) -> String {
        "  \(title): current \(number(value.current, suffix: unit)); "
