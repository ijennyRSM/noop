...
///
/// Stored day keys are Gregorian `yyyy-MM-dd` identifiers. Never parse them
/// using `Calendar.current` because the user calendar may be Buddhist or
/// another non-Gregorian calendar.
public enum CanonicalDay {
    ...
    public static func startOfDay(
        ...
        guard let parsed = date(from: key, timeZone: timeZone) else {
            return nil
        }
    ...
    public static func isInWindow(
        ...
        guard let day = startOfDay(for: key, timeZone: timeZone) else {
            return false
        }
        let calendar = calendar(timeZone: timeZone)
    ...
    public static func startOfWindow(
        daysIncludingToday days: Int,
        now: Date,
        timeZone: TimeZone = .current
    ) -> Date? {
