...
final class CanonicalDayTests: XCTestCase {
    ...
    func testStrictParsingRejectsMalformedAndImpossibleKeys() {
        XCTAssertNil(CanonicalDay.date(
            from: "2026-2-3",
            timeZone: bangkok
        ))
        XCTAssertNil(CanonicalDay.date(
    ...
    func testJulyWindowsHaveExactCalendarBoundaries() throws {
        ...
        XCTAssertEqual(
            CanonicalDay.key(for: seven, timeZone: bangkok),
            "2026-07-22"
        )
        XCTAssertEqual(
            CanonicalDay.key(for: twentyEight, timeZone: bangkok),
            "2026-07-01"
        )
        XCTAssertEqual(
            CanonicalDay.key(for: priorThirty, timeZone: bangkok),
            "2026-06-28"
        )
    }
}
