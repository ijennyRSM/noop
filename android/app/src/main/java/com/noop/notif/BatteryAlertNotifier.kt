...
object BatteryAlertNotifier {
    ...
    fun onRuntimeEstimate(context: Context, remainingHours: Double?, charging: Boolean?) {
        ...
        runCatching {
            ...
            val title = context.getString(R.string.battery_runtime_title)
            val body = context.getString(R.string.battery_runtime_body, label)
            AlertInbox.post(context, AlertInbox.Kind.BATTERY_RUNTIME, title, body)
    ...
    fun onBatteryUpdate(context: Context, currPct: Int?, charging: Boolean?) {
        ...
        // Advance local state and the in-app alert history even when system banners are unavailable.
        ...
        if (decision.fireLow) {
            AlertInbox.post(
                context, AlertInbox.Kind.BATTERY_LOW,
        ...
        if (decision.fireFull) {
            AlertInbox.post(
                context, AlertInbox.Kind.BATTERY_FULL,
    ...
    fun onCriticalBattery(context: Context, currPct: Int?, charging: Boolean?) {
        ...
        runCatching {
            ...
            if (!decision.fire) return
            val title = context.getString(R.string.battery_critical_title)
            val body = context.getString(R.string.battery_critical_body, currPct)
            AlertInbox.post(context, AlertInbox.Kind.BATTERY_CRITICAL, title, body)
            if (!NotificationManagerCompat.from(context).areNotificationsEnabled()) return
            ensureChannel(context)
            val n = NotificationCompat.Builder(context, CHANNEL_ID)
                .setSmallIcon(R.drawable.ic_stat_heart)
    ...
    fun onBedtimeRunway(
        ...
        runCatching {
            ...
            val title = context.getString(R.string.battery_bedtime_title)
            ...
            AlertInbox.post(context, AlertInbox.Kind.BATTERY_BEDTIME, title, body)
            if (!NotificationManagerCompat.from(context).areNotificationsEnabled()) return
            ensureChannel(context)
            val n = NotificationCompat.Builder(context, CHANNEL_ID)
                .setSmallIcon(R.drawable.ic_stat_heart)
